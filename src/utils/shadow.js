...
function makeShadowRaw(
  rootEl,
  childNodes,
  { mode = 'open', delegatesFocus = false } = { mode: 'open' },
) {
  try {
...
const ShadowStyle = vue.defineComponent({
  ...
  setup(props, { slots }) {
    return () =>
      vue.h(
        'style',
        { media: props.media, nonce: props.nonce },
        slots.default?.(),
      );
  },
});
const ShadowRoot = withType()(
  vue.defineComponent({
    ...
    setup(props, { slots, expose, emit }) {
      ...
      const teleport_target = vue.computed(
        () => shadow_root.value ?? virtual_root,
      );
        ...
        try {
          if (abstract.value) {
            if (teleport_el.value.parentElement.shadowRoot) {
              ...
              shadow_root.value = makeShadowRaw(
                teleport_el.value.parentElement,
                void 0,
                {
                  mode: props.mode,
            ...
            shadow_root.value = makeShadowRaw(el.value, void 0, {
              mode: props.mode,
              delegatesFocus: props.delegatesFocus,
            });
          }
      ...
      vue.watch(
        [shadow_root, () => props.adoptedStyleSheets],
      ...
      return () => {
        const child_part = vue.h(
          vue.Teleport,
          { ref: teleport_el, to: teleport_target.value },
          [slots.default?.()],
        );
...
function install(app) {
  app.component('ShadowRoot', ShadowRoot);
  app.directive('shadow', {
    beforeMount(el) {
      console.warn(
        '[ShadowDom] Deprecated v-shadow directive, use <shadow-root> component',
      );
      makeShadow(el);
    },
  });
...
export { ShadowRoot, ShadowStyle, makeShadow, makeShadowRaw, install };
