# syntax = docker/dockerfile:1
ARG NODE_VERSION="18.19.1"
ARG BASE_VERSION="alpine3.19"
ARG OLD_IMAGE="connectof/connect-webapp:latest"

# Add build arguments for env variables
ARG VERSION_NUMBER
ARG ROOT_API
ARG CHATS_API_URL
ARG BILLING_API_URL
ARG KEYCLOAK_ISSUER
ARG KEYCLOAK_CLIENT_ID
ARG KEYCLOAK_REALM
ARG SENTRY_ENVIRONMENT
ARG HELPHERO
ARG STRIPE_API
ARG BOT_URL
ARG TWO_FA_APP_ANDROID
ARG TWO_FA_APP_IOS
ARG LOGROCKET_ID
ARG HOTJAR_ID
ARG FLOWS_GENERIC_TOKEN
ARG MODULES_YAML
ARG NEXUS_API
ARG PROJECTS_BOTHUB_ALLOWED
ARG MODULE_FEDERATION_REMOTE_URL
ARG LOGROCKET_CHILD_DOMAINS
ARG SENTRY_DSN_ENDPOINT
ARG GITHUB_API
ARG GITHUB_CONTENT_API
ARG GITHUB_PLATFORM_UPDATES_REPOSITORY
ARG GROWTHBOOK_CLIENT_KEY
ARG GROWTHBOOK_API_HOST
ARG MODULE_FEDERATION_INSIGHTS_URL
ARG MODULE_FEDERATION_INTEGRATIONS_URL
ARG MODULE_FEDERATION_BULK_SEND_URL
ARG MODULE_FEDERATION_AGENT_BUILDER_URL
ARG MODULE_FEDERATION_CHATS_URL
ARG PUBLIC_PATH_URL
ARG APPOINTMENT_LINK_FOR_AGENT_BUILDER_2_0_TRIAL
ARG APPOINTMENT_LINK_FOR_AGENT_BUILDER_2_0_NON_TRIAL

FROM node:${NODE_VERSION}-${BASE_VERSION} AS builder

WORKDIR /home/app

# Add them again due to new docker build layer
ARG VERSION_NUMBER
ARG ROOT_API
ARG CHATS_API_URL
ARG BILLING_API_URL
ARG KEYCLOAK_ISSUER
ARG KEYCLOAK_CLIENT_ID
ARG KEYCLOAK_REALM
ARG SENTRY_ENVIRONMENT
ARG HELPHERO
ARG STRIPE_API
ARG BOT_URL
ARG TWO_FA_APP_ANDROID
ARG TWO_FA_APP_IOS
ARG LOGROCKET_ID
ARG HOTJAR_ID
ARG FLOWS_GENERIC_TOKEN
ARG MODULES_YAML
ARG NEXUS_API
ARG PROJECTS_BOTHUB_ALLOWED
ARG MODULE_FEDERATION_REMOTE_URL
ARG LOGROCKET_CHILD_DOMAINS
ARG SENTRY_DSN_ENDPOINT
ARG GITHUB_API
ARG GITHUB_CONTENT_API
ARG GITHUB_PLATFORM_UPDATES_REPOSITORY
ARG GROWTHBOOK_CLIENT_KEY
ARG GROWTHBOOK_API_HOST
ARG MODULE_FEDERATION_INSIGHTS_URL
ARG MODULE_FEDERATION_INTEGRATIONS_URL
ARG MODULE_FEDERATION_BULK_SEND_URL
ARG MODULE_FEDERATION_AGENT_BUILDER_URL
ARG MODULE_FEDERATION_CHATS_URL
ARG PUBLIC_PATH_URL
ARG APPOINTMENT_LINK_FOR_AGENT_BUILDER_2_0_TRIAL
ARG APPOINTMENT_LINK_FOR_AGENT_BUILDER_2_0_NON_TRIAL
# Set them as environment variables
ENV VERSION_NUMBER=${VERSION_NUMBER}
ENV ROOT_API=${ROOT_API}
ENV CHATS_API_URL=${CHATS_API_URL}
ENV BILLING_API_URL=${BILLING_API_URL}
ENV KEYCLOAK_ISSUER=${KEYCLOAK_ISSUER}
ENV KEYCLOAK_CLIENT_ID=${KEYCLOAK_CLIENT_ID}
ENV KEYCLOAK_REALM=${KEYCLOAK_REALM}
ENV SENTRY_ENVIRONMENT=${SENTRY_ENVIRONMENT}
ENV HELPHERO=${HELPHERO}
ENV STRIPE_API=${STRIPE_API}
ENV BOT_URL=${BOT_URL}
ENV TWO_FA_APP_ANDROID=${TWO_FA_APP_ANDROID}
ENV TWO_FA_APP_IOS=${TWO_FA_APP_IOS}
ENV LOGROCKET_ID=${LOGROCKET_ID}
ENV HOTJAR_ID=${HOTJAR_ID}
ENV FLOWS_GENERIC_TOKEN=${FLOWS_GENERIC_TOKEN}
ENV MODULES_YAML=${MODULES_YAML}
ENV NEXUS_API=${NEXUS_API}
ENV PROJECTS_BOTHUB_ALLOWED=${PROJECTS_BOTHUB_ALLOWED}
ENV MODULE_FEDERATION_REMOTE_URL=${MODULE_FEDERATION_REMOTE_URL}
ENV LOGROCKET_CHILD_DOMAINS=${LOGROCKET_CHILD_DOMAINS}
ENV SENTRY_DSN_ENDPOINT=${SENTRY_DSN_ENDPOINT}
ENV GITHUB_API=${GITHUB_API}
ENV GITHUB_CONTENT_API=${GITHUB_CONTENT_API}
ENV GITHUB_PLATFORM_UPDATES_REPOSITORY=${GITHUB_PLATFORM_UPDATES_REPOSITORY}
ENV GROWTHBOOK_CLIENT_KEY=${GROWTHBOOK_CLIENT_KEY}
ENV GROWTHBOOK_API_HOST=${GROWTHBOOK_API_HOST}
ENV MODULE_FEDERATION_INSIGHTS_URL=${MODULE_FEDERATION_INSIGHTS_URL}
ENV MODULE_FEDERATION_INTEGRATIONS_URL=${MODULE_FEDERATION_INTEGRATIONS_URL}
ENV MODULE_FEDERATION_BULK_SEND_URL=${MODULE_FEDERATION_BULK_SEND_URL}
ENV MODULE_FEDERATION_AGENT_BUILDER_URL=${MODULE_FEDERATION_AGENT_BUILDER_URL}
ENV MODULE_FEDERATION_CHATS_URL=${MODULE_FEDERATION_CHATS_URL}
ENV PUBLIC_PATH_URL=${PUBLIC_PATH_URL}
ENV APPOINTMENT_LINK_FOR_AGENT_BUILDER_2_0_TRIAL=${APPOINTMENT_LINK_FOR_AGENT_BUILDER_2_0_TRIAL}
ENV APPOINTMENT_LINK_FOR_AGENT_BUILDER_2_0_NON_TRIAL=${APPOINTMENT_LINK_FOR_AGENT_BUILDER_2_0_NON_TRIAL}
RUN apk --no-cache add git

COPY package.json package-lock.json ./

RUN --mount=type=cache,target=/root/.npm NPM_CONFIG_CACHE=/root/.npm npm install

COPY . ./

RUN NODE_OPTIONS=--openssl-legacy-provider npm run build

FROM ${OLD_IMAGE} AS old_css

FROM nginxinc/nginx-unprivileged:1.25-alpine

COPY --chown=nginx:nginx nginx.conf /etc/nginx/nginx.conf
COPY --chown=nginx:nginx headers /usr/share/nginx/html/headers

COPY --from=builder --chown=nginx:nginx /home/app/dist /usr/share/nginx/html/connect/
COPY --from=old_css --chown=nginx:nginx /usr/share/nginx/html/connect/assets/all.tx[t] /usr/share/nginx/html/connect/assets/*.css /usr/share/nginx/html/connect/assets/

COPY docker-entrypoint.sh file_handler.sh /

RUN cd /usr/share/nginx/html/connect/ \
    && /file_handler.sh css

COPY --from=builder --chown=nginx:nginx /home/app/dist /usr/share/nginx/html/connect/

RUN mv /usr/share/nginx/html/connect/index.html /usr/share/nginx/html/connect/index.html.tmpl \
    && cd /usr/share/nginx/html/connect/ \
    && ln -s /tmp/index.html

EXPOSE 8080
ENTRYPOINT ["/docker-entrypoint.sh"]
CMD ["nginx", "-g", "daemon off;"]
